...
def change_node_name(G, old_name, new_name):
    # Copy the attributes of the old node to the averages node
    attributes = G.nodes[old_name]
    G.add_node(new_name, **attributes)

    # Iterate through the edges of the old node and add equivalent edges with the averages node
