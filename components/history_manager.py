...
def update_and_merge_graphs(EX, MP):
    """
    ...
    for node in EX.nodes():
        if MP is not None and node in MP:
            # Increment frequency attribute if it exists, otherwise initialize it
            EX.nodes[node]['frequency'] = EX.nodes[node].get('frequency', 0) + 1

    # Add nodes and edges from E that don't exist in H
    for node in MP.nodes():
        if node not in EX:
            EX.add_node(node, **MP.nodes[node])

    for edge in MP.edges(data=True):
        if not EX.has_edge(*edge[:2]):
            EX.add_edge(*edge[:2], **edge[2])
